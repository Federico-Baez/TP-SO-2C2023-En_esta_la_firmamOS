...
void deteccion_deadlock(){
	while(1){
		...
		if(list_size(lista_posibles_deadlocks_recurso) > 1){
			...
			while(1){
				...
				if(esta_pcb_en_una_lista_especifica(lista_posibles_deadlocks_recurso, recurso->pcb_asignado)){
					...
					while(!list_is_empty(lista_pcbs_en_deadlock)){
						t_pcb* un_pcb = list_remove(lista_pcbs_en_deadlock, 0);
			...
			if(hay_ciclo){
				log_info(kernel_logger, "La lista tiene: %d -- ELEMENTOS", list_size(lista_pcbs_en_deadlock));
...
void logear_proceso_en_deadlock(t_pcb* pcb){
	...
	if(!list_is_empty(pcb->archivos_abiertos)){
		string_append(&log_proceso," - Archivos en posesión: ");
		for(int j = 0; j < list_size(pcb->archivos_abiertos); j++){
			t_archivo_abierto_pcb* archivo_pcb = list_get(pcb->archivos_abiertos, j);
			string_append(&log_proceso,archivo_pcb->nombre_archivo);
			if(j == list_size(pcb->archivos_abiertos)-1){
